...
import { ref, get, child, set, push, query, orderByChild, equalTo, serverTimestamp } from 'firebase/database';
...
const BookAppointment = () => {
    ...
    const fetchTeachers = async () => {
      try {
        ...
        if (teacherSnapshot.exists()) {
          ...
          const formattedTeachers = Object.entries(teacherData).map(([id, teacher]) => ({
            value: id,
            lable: teacher.fullName || `${teacher.firstName} ${teacher.lastName}`
          }));
  ...
  const isDateAvailable = (checkDate) => {
    ...
    const availableSlots = generateTimeSlots().filter(slot => 
      !bookedSlots[dateString].includes(slot)
    );
  ...
  const handleDateChange = (event, selectedDate) => {
    ...
    if (selectedDate) {
      ...
      if (!isDateAvailable(selectedDate)) {
        setDateError(true);
        Alert.alert("Date Unavailable", "No available time slots on this date.");
        return;
      }
  ...
  const handleAppointmentSubmit = async () => {
    ...
    try {
      ...
      const teacherName = selectedTeacherData ? selectedTeacherData.lable : 'Selected Teacher';
      ...
      console.error('Appointment submission error:', error, `Time until error: ${errorTime.toFixed(2)}ms`);
    ...
    <ScrollView style={styles.container}>  
      ...
      <View style={styles.section}>
          ...
          <View style={[
            ...
            teacherError && styles.errorBorder
          ]}>
            <SelectCountry
          ...
          {teacherError && <Text style={styles.errorText}>Please select a teacher</Text>}
        </View>
      <View style={styles.section}>
          ...
          {dateError && (
            <Text style={styles.errorText}>
              {date && !isWeekday(date) ? 'Invalid date. Please select another date.' : 'Please select a date'}
            </Text>
          )}
          {showDatePicker && (
            <DateTimePicker
        ...
        {timeError && <Text style={styles.errorText}>Please select a time slot</Text>}
      </View>
        ...
        <View style={styles.section}>
          ...
          <View style={[
            ...
            >
              <Text style={[styles.typeText, appointmentType === 'Academic' && styles.selectedTypeText]}>
                Academic
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              <Text style={[styles.typeText, appointmentType === 'Personal' && styles.selectedTypeText]}>
                Personal
              </Text>
            </TouchableOpacity>
          </View>
          {typeError && <Text style={styles.errorText}>Please select an consultation type</Text>}
        </View>
        ...
        <View style={styles.section}>
          ...
          />
          {reasonError && <Text style={styles.errorText}>Please provide a reason for the consultation</Text>}
        </View>
